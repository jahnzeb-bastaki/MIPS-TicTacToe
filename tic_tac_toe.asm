...
instruction6:		.asciiz "Input whether you would like to play as X or O: "
instruction7:		.asciiz "\n\nPLAYER is "
instruction8:		.asciiz "COMPUTER is "
instruction9:		.asciiz "\n\nERROR: The symbol that you have chosen is not a X or O, please try again\n\n"
userprompt1:		.asciiz "\nPLAYER's turn. Pick a space with its corresponding number: "
userprompt2:		.asciiz "\nPLAYER has put a X in cell "
invalidprompt1:		.asciiz "\nPLAYER input is not valid. Please pick a number from 1 to 9\nPlease try again\n\n"
...
OLoop:	jal	computerInput
	jal	displayBoard
	# check to see winner, if there is a winner, game will end in winner
	jal	checkWin
...
readC:
	la	$a0, instruction6
	syscall
	li	$v0, 12		# load v0 to read a character
	syscall
	move	$t0, $v0	# move the character into t0
	
	beq	$t0, 88, picksX	# if the user picks X jump to picksX
	beq	$t0, 79, picksO	# if the user picks O jump to picksO
	
	li	$v0, 4
	la	$a0, instruction9
	syscall
	j	readC		# jump back to readC
	
picksX:
	li	$s7, 88		# PLAYER input
	li	$s6, 79		# COMPUTER input
	j	exit
	
picksO:
	li	$s7, 79		# PLAYER input
	li	$s6, 88	        # COMPUTER input
	j	exit
exit:
	# print instruction 7
	li	$v0, 4
	...
	la	$a0, breakline
	syscall	
	beq	$s7, 88, XLoop	# if the user picks X jump to XLoop
	beq	$s7, 79, OLoop	# if the user picks O jump to OLoop
...
checkWinner:
	...
	beq	$t1, $s7, playerWins
	beq	$t1, $s6, computerWins
